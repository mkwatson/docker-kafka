Fork of spotify/kafka with versions bumped
...
```bash
docker run -p 2181:2181 -p 9092:9092 --env ADVERTISED_HOST=`docker-machine ip \`docker-machine active\`` --env ADVERTISED_PORT=9092 mkwatson/kafka
```
...
Take the same parameters as the mkwatson/kafka image with some new ones:
...
https://registry.hub.docker.com/u/mkwatson/kafka/

https://registry.hub.docker.com/u/mkwatson/kafkaproxy/

Build from Source
---

    docker build -t mkwatson/kafka kafka/
    docker build -t mkwatson/kafkaproxy kafkaproxy/
